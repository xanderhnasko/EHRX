...
class Pager:
    ...
    def rasterize_page_detached(self, page_num: int, dpi: int = 150) -> Tuple[np.ndarray, PageInfo]:
        """Expose rasterizer.rasterize_page for callers that need a one-off DPI."""
        return self.rasterizer.rasterize_page(page_num, dpi)

    def rasterize_page_multi(self, page_num: int, detection_dpi: int = 200, ocr_dpi: int = 300) -> Tuple[Tuple[np.ndarray, PageInfo], Tuple[np.ndarray, PageInfo]]:
        """Rasterize once for detection and once for OCR."""
        det_img, det_info = self.rasterizer.rasterize_page(page_num, detection_dpi)
        ocr_img, ocr_info = self.rasterizer.rasterize_page(page_num, ocr_dpi)
        return (det_img, det_info), (ocr_img, ocr_info)

    @staticmethod
    def crop_image(image: np.ndarray, bbox_px: List[int], pad_px: int = 0) -> np.ndarray:
        """Crop region with padding and bounds checks."""
        h, w = image.shape[:2]
        x0 = max(0, int(bbox_px[0]) - pad_px)
        y0 = max(0, int(bbox_px[1]) - pad_px)
        x1 = min(w, int(bbox_px[2]) + pad_px)
        y1 = min(h, int(bbox_px[3]) + pad_px)
        if x1 <= x0 or y1 <= y0:
            return image[0:0, 0:0]
        return image[y0:y1, x0:x1]

    def extract_vector_text_in_bbox(self, page_num: int, bbox_pdf: List[float]) -> str:
        """Concatenate vector text spans whose PDF bboxes overlap the given bbox."""
        spans = self.get_page_vector_text(page_num)
        x0, y0, x1, y1 = bbox_pdf
        texts: List[str] = []
        for s in spans:
            sb = s.get("bbox", [0, 0, 0, 0])
            # overlap check in PDF space
            if not (sb[2] < x0 or sb[0] > x1 or sb[3] < y0 or sb[1] > y1):
                t = (s.get("text") or "").strip()
                if t:
                    texts.append(t)
        return " ".join(texts)

    # (Removed legacy additive helpers from previous implementation to avoid name errors)
